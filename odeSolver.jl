...
# ╔═╡ ce035857-9ec3-4942-8ff4-bc3f39cc5687
begin
...
# ╔═╡ 588376cc-c654-4782-ad4e-6ea9fcb4491f
begin
N=34
Rd=4.282;
rhat=range(0,Rd,500);
...
# ╔═╡ afcd3f04-b970-4e6d-9741-22ce37cdbf46
(ω,kn)

# ╔═╡ 60e0b33e-5c32-479c-ba10-ce6975cc9e8f
begin
x0=zeros(2*N,1);
for n=1:N
fun(x)=-epsn*x.*(1-(x/b).^2).*exp(-(x/b).^2).*besselj.(0,kn[n].*x);
den(x)= x.*besselj.(0,kn[n]*x).*besselj.(0,kn[n]*x);
x0[n]=quadgk(fun,0,1,rtol=1e-3)[1]/quadgk(den,0,1,rtol=1e-3)[1];
end
end

# ╔═╡ 30ce9fb5-4c20-4bb4-bde9-276a8d328e4e

function nayfeh!(du, u, p, t)
print("time=%f\n",float(t))
   for n = 1:N
	    sum = 0;
	    Ga=h5read("./hd5_newdir/Ga_$n.h5","mygroup2/Ga")
	    Ch=h5read("./hd5_newdir/Ch_$n.h5","mygroup2/Ch")
	
	
		sum=u[1:N,1]'*Ga[1:N,1:N]*(-(ω .^2).*u[1:N,1])+u[N+1:2*N]'*Ch[1:N,1:N]*u[N+1:2*N];
	
	
	
	    du[n]=u[n+N];
	    du[n+N]=-ω[n]^2*u[n]-sum;
...
# ╔═╡ 82c6c87f-461a-4a56-b3cf-ade52a153ec6


# ╔═╡ 469f95b9-583d-420c-a1b2-b6c7f7d5967f
begin
	tspan=(0,t0n[end]);
    prob=ODEProblem(nayfeh!, x0, tspan)
	sol=solve(prob, saveat = dt);
end

# ╔═╡ f981ce70-ad30-4c82-919e-33cd4292e153
writedlm("new_nonlin.dat", sol.u)

# ╔═╡ cca2edd0-685b-47b0-9a42-aeb67a3b6b00
# ╠═╡ disabled = true
#=╠═╡
begin
 etan = zeros(length(Y),1);
p=plot()
 end
  ╠═╡ =#

# ╔═╡ 4ce6083f-68a3-4d59-a811-b58675af884f
# ╠═╡ disabled = true
#=╠═╡
 begin
for t in 1:length(sol.t)
	for ss in 1:length(Y)
		 summ=0;
		for ll in 1:N
			summ = summ+ besselj.(0,kn[ll]*Y[ss])*sol.u[t][ll];
		end
		etan[ss]=summ
	end
	p= plot(Y,etan)
...
# ╔═╡ 2c0df57e-c3bd-4800-939b-540d9080c21c
...
manifest_format = "2.0"
project_hash = "1a4a190311d792f14007e7ffe71fbc6b002f5ec4"
